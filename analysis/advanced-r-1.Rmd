...
<!-- TODO: bugfix: the figure below renders incorrectly,
     ...
     review build_analysis_site() conversion code
  -->
...
Answer: `appendLF` controls whether or not a newline is added to the message; by default, `cat()`
...
Answer: `abort()` includes the error message and backtrace in the condition object. `abort()` uses
the custom class `rlang_error`.
...
Answer: predictions in comments. The prediction for `show_condition(10)` was incorrect; the correct
answer is `NULL` as that's what's returned when there is no condition. The final prediction was also
incorrect; the `tryCatch()` call stops further execution when the `message()` is caught.
...
Answer: code above. The question specifies to use `quietly = FALSE`, but this approach makes more
sense (to me). The alternative is to catch the 'Failed with error:' message which seems less
reliable.
...
Notes on comparing exercise solutions here against
