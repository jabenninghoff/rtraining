...
**Important:** advice on tidy evaluation has changed since these chapters were written; see the "Programming with dplyr" [vignette](https://dplyr.tidyverse.org/articles/programming.html).
...
bc <- function(lambda) {
  ...
  new_function(
    exprs(x = ), # nolint: missing_argument_linter, spaces_inside_linter.
    func,
    caller_env()
  )
}
...
bc2 <- function(lambda) {
  ...
  if (!!lambda == 0) {
    new_function(exprs(x = ), expr(log(x))) # nolint: missing_argument_linter, spaces_inside_linter.
  } else {
    # nolint start: missing_argument_linter, spaces_inside_linter.
...
```{r eval = FALSE}
compose <- function(f, g) { # nolint: object_usage_linter.
  new_function(
    expr(...),
    expr({
      (!!f)((!!g)(...))
...
```{r}
compose2 <- function(f, g) { # nolint: object_usage_linter.
  f <- enexpr(f)
  g <- enexpr(g)
...
locate_evaluation <- function(file, local) {
  x <- "local environment" # nolint: object_usage_linter.
  source(file, local = local) # nolint: undesirable_function_linter.
}
...
AR Solutions: Let's look at a quote from the [first edition of *Advanced R*](http://adv-r.had.co.nz/Computing-on-the-language.html#subset):
...
assign2 <- function(name, value, env) { # nolint: object_usage_linter. `!!` is OK.
  s <- sym(name)
...
```{r}
assign2 <- function(name, value, env = caller_env()) { # nolint: object_usage_linter. `!!` is OK.
  name_sym <- sym(name)
