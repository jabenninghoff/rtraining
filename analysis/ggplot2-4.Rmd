...
ggplot(diamonds, aes(carat, price)) +
  geom_point(aes(color = color)) +
  geom_smooth(data = diamonds2, method = "rlm", formula = y ~ x, se = FALSE) +
  coord_cartesian(ylim = c(0, max(diamonds$price))) +
  facet_wrap(~color)
```
...
```{r}
mpg2c <- select(mpg2, -class)
ggplot(mpg2, aes(displ, hwy)) +
  geom_point() +
  geom_smooth(data = mpg2c, method = "loess", formula = y ~ x, se = FALSE) +
  facet_wrap(~class)
```
