...
4.  The Pew Research Center's favorite data visualizations of 2014:
    [link](https://www.pewresearch.org/fact-tank/2014/12/29/our-favorite-pew-research-center-data-visualizations-from-2014/)
...
```{r}
ggplot(ggplot2::mpg, aes(class, cty)) +
  geom_point() +
  coord_flip()

ggplot(ggplot2::mpg, aes(trans, hwy)) +
  geom_boxplot() +
  coord_flip()
```
...
```{r}
mod <- loess(hwy ~ displ, data = ggplot2::mpg)
...
```{r}
ggplot(ggplot2::mpg, aes(drv, trans)) +
...
```{r}
mpg2 <- subset(ggplot2::mpg, cyl != 5 & drv %in% c("4", "f") & class != "2seater")
...
```{r}
# Need to use dplyr::select for disambiguation for build_analysis_site()
ggplot(ggplot2::mpg, aes(displ, hwy)) +
...
Comparing to the [source code](https://github.com/hadley/ggplot2-book/blob/master/facet.Rmd), the answer is correct!
...
```{r}
base <- ggplot(ggplot2::mpg, aes(displ, hwy)) +
